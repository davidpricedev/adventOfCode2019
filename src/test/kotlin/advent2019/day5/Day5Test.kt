...
class Day5Test : FreeSpec(
    {
        "Part1" - {
            "handles parameter modes" {
                ...
                ICComp(inputStringToList(input)).run().memoryAsString() shouldBe "1002,4,3,4,99"
            }
            "handles negative values" {
                val input = "1101,100,-1,4,0"
                ICComp(inputStringToList(input)).run().memoryAsString() shouldBe "1101,100,-1,4,99"
            }
            "handles input and output" {
                val program = "3,0,4,0,99"
                val programList = inputStringToList(program)
                val comp = ICComp(programList, inputs = listOf(10))
                val result = comp.run()
